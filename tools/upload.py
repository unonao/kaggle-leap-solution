...
def copy_files_with_exts(source_dir: Path, dest_dir: Path, exts: list):
    ...
    for ext in exts:
        for source_path in source_dir.rglob(f"*{ext}"):
            ...
            file_name = 'kami_experiments_' + '_'.join(str(relative_path).split('/'))
            print(file_name)
...
@click.command()
@click.option("--title", "-t", default="leap-verify-pred")
@click.option("--dir", "-d", type=Path, default="./output/experiments")
@click.option(
    "--extentions", "-e", type=list[str], default=["submission.parquet"]
)
